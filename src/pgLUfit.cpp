...
void pgLUfit<TX>::pgLUfit_main(){
    ...
    double fVal_k(0);
    VectorXd fVal_all_k;fVal_all_k.setZero();
    bool converged(false);
        if(verbose){Rcpp::Rcout<<"Fitting "<<k<<"th lambda\n";}
        switch(method_int){
            case 1:
                std::tie(beta,fVal_k,subgrad_k,fVal_all_k,iter,converged) = GD(f,g,N,q,beta,stepSize,maxit,ST,subgradient,lambda_k,tol,trace);
                break;
            case 2:
                for(int i=0;i<maxit;i++){stepSizeSeq(i) = stepSize/(1.0+stepSize*lambdaseq(k)*i);}
                std::tie(beta,fVal_k,subgrad_k,fVal_all_k,iter,converged) = SGD(f,g,q,beta,samplingProbabilities,stepSizeSeq,batchSize,maxit,ST,subgradient,lambda_k,tol,trace);
                break;
            case 3:
                 std::tie(beta,fVal_k,subgrad_k,fVal_all_k,iter,converged) = SVRG(f,g,q,beta,samplingProbabilities,stepSize,N,batchSize,maxit,ST,subgradient,lambda_k,tol,trace);
                break;
            case 4:
                std::tie(beta,fVal_k,subgrad_k,fVal_all_k,iter,converged)= SAG(f,g,q,beta,samplingProbabilities,stepSize,batchSize,maxit,ST,subgradient,lambda_k,true,tol,trace);
                break;
            default:
                std::tie(beta,fVal_k,subgrad_k,fVal_all_k,iter,converged) = GD(f,g,N,q,beta,stepSize,maxit,ST,subgradient,lambda_k,tol,trace);
                break;}
