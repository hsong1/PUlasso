    ...
    :X(X_),z(z_),icoef(icoef_), gsize(gsize_), pen(pen_),lambdaseq(lambdaseq_), isUserLambdaseq(isUserLambdaseq_),pathLength(pathLength_),lambdaMinRatio(lambdaMinRatio_),pi(pi_),maxit(maxit_), tol(tol_),inner_tol(inner_tol_),useStrongSet(useStrongSet_),verbose(verbose_),nfolds(nfolds_),nfits(nfits_),ncores(ncores_),lu_f(X_, z_, icoef_, gsize_,pen_,lambdaseq_,isUserLambdaseq_,pathLength_,lambdaMinRatio_,pi_,maxit_,tol_,inner_tol_,useStrongSet_,verbose_,false)
    {
    ...
    void cv_LUfit<TX>::cv_LUfit_main()
    {
        ...
        if(verbose){Rcout<<"Starting cross-validation"<<endl;}
    ...
    #pragma omp parallel for shared(nullDev_,Deviances_,coefMat_) schedule(dynamic) reduction(+: errCount)
            for (int j=0;j<std::min(nfolds,nfits);++j)
            {
                ...
                LUfit<MatrixXd> lu_t(X_lu_t, z_lu_t, icoef, gsize,pen,lambdaseq,true,pathLength,lambdaMinRatio,pi,maxit,tol,inner_tol,useStrongSet,false,false);
                lu_t.LUfit_main();
                lu_t.decenterX();
    ...
    #pragma omp critical
        ...
        if(verbose){Rcout<<"Fitting full data"<<endl;}
    ...
    #pragma omp parallel for shared(nullDev_,Deviances_,coefMat_) schedule(dynamic) reduction(+: errCount)
        for (int j=0;j<std::min(nfolds,nfits);++j)
        {
            ...
            try
            {
                ...
                LUfit<SparseMatrix<double> > lu_t(X_lu_t, z_lu_t, icoef, gsize,pen,lambdaseq,true,pathLength,lambdaMinRatio,pi,maxit,tol,inner_tol,useStrongSet,false,false);
