...
#'@useDynLib PUlasso
#'@param X Input matrix; each row is an observation. Can be in matrix or sparse matrix format.
                      ...
                      verbose = FALSE,nfolds=10,nfits=nfolds,nCores=0)
{
  ...
  g<-cv_LU_cpp(X_ = X_lu_int,z_ = z_lu,icoef_ = icoef,gsize_ = gsize,pen_ = pen,
            ...
            isSparse = is.sparse,verbose_ = verbose,nfolds_ = nfolds,nfits_ = nfits, ncores_ = nCores)
  #
  ...
  # result<-structure(list(cvm=cvm,cvsd=cvsd, lambda = g$lambda, lambda.min= lambda.min, 
  #                         lambda.1se=lambda.1se,perm.ind = perm.ind),class="cvPUfit")
  return(result)
}
